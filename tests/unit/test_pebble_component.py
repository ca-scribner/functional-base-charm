...
import functional_base_charm.pebble_component
from functional_base_charm.pebble_component import PebbleComponent

from fixtures import MinimalPebbleComponent, MinimalPebbleServiceComponent, harness, harness_with_container
...
class TestPebbleComponent:
    ...
    def test_ready_for_execution_if_service_up(self, harness_with_container):
        ...
        pc = MinimalPebbleComponent(charm=harness_with_container.charm, container_name=self.container_name)
...
class TestPebbleServiceComponent:
    ...
    def test_configure_charm(self, harness_with_container):
        ...
        """
        harness_with_container.set_can_connect(self.container_name, True)
        pc = MinimalPebbleServiceComponent(charm=harness_with_container.charm, container_name=self.container_name, service_name="test-service")
        services_expected = pc.get_layer().services

        pc.configure_charm("mock event")

        services_actual = harness_with_container.get_container_pebble_plan(self.container_name).services
    ...
    def test_configure_charm_if_container_not_ready(self, harness_with_container):
        ...
        with mock.patch.object(functional_base_charm.pebble_component, "logging") as mock_logger:
            pc.configure_charm("mock event")
    ...
    def test_get_services_not_active_if_container_ready_services_not_started(self, harness_with_container):
    ...
    def test_get_services_not_active_if_container_ready_services_active(self, harness_with_container):
